...
## 37. 2026-06-05 架构调整：无连接上位机通道

由于当前 SDK 未确认支持“连接态 + 扩展广播 + 扫描”稳定共存，上位机通信从 GATT 连接模式调整为广播扫描模式。

调整后的原则：

- 上位机不再必须连接吊坠。
- 吊坠始终处于 Extended Advertising + Scanning 工作模式。
- 上位机命令、设备响应、日志、事件都通过扩展广播承载。
- 原 `app_host_cmd` 命令业务层保留，传输层由 `app_host_transport` 统一调度。
- GATT 调试服务保留为兼容实现，但默认通过 `APP_BLE_ENABLE_GATT_DEBUG = 0` 关闭连接入口。

新增模块：

| 模块 | 目录 | 说明 |
| --- | --- | --- |
| `app_host_adv` | `pendant/host_adv/` | 广播上位机通道，负责 HOST-ADV 分片、重组、去重 |
| `app_host_transport` | `pendant/host_transport/` | Host 消息统一发送出口，当前优先使用广播通道 |

广播链路：

```text
上位机扩展广播
  -> app.c controller_event_callback
  -> app_scan_on_report
  -> app_adv_proto_decode
  -> app_host_adv_on_adv_frame
  -> app_host_adv_poll
  -> app_host_cmd_on_rx_message
  -> 业务命令处理
  -> app_host_transport_send_message
  -> app_host_adv_send_message_with_seq
  -> app_adv_scheduler_enqueue_frame
  -> app_ble_update_ext_adv_data
```

当前限制：

- 第一版仅通过重复广播提高可达性，尚未实现 ACK/重传。
- Windows Bleak 通常不能发送自定义扩展广播，PC GUI 暂未切换到广播控制。
- 后续需要明确上位机实际 BLE 后端：Android、HCI dongle 或专用调试器。
